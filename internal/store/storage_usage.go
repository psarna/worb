...
func (db *DB) ApproximateRunStorageUsageSnapshot() StorageUsageSnapshot {
	rows, err := db.Query(`
		SELECT
			r.id,
			r.project_id,
		...
		LEFT JOIN (
			SELECT run_id, COUNT(*) * 40 AS bytes
			FROM history_scalars
			GROUP BY run_id
		) hs ON hs.run_id = r.id
		LEFT JOIN (
			SELECT run_id, COUNT(*) * 256 AS bytes
		...
		LEFT JOIN (
			SELECT run_id, COUNT(*) * 160 AS bytes
			FROM console_logs
			GROUP BY run_id
		...
		LEFT JOIN (
			SELECT run_id, COUNT(*) * 96 AS bytes
			FROM files
			GROUP BY run_id
		) f ON f.run_id = r.id
		LEFT JOIN (
			SELECT run_id, COUNT(*) * 32 AS bytes
			FROM run_keys
			GROUP BY run_id
		) rk ON rk.run_id = r.id
		WHERE r.deleted_at IS NULL AND p.deleted_at IS NULL
		ORDER BY total_bytes DESC, p.name ASC, COALESCE(NULLIF(r.display_name, ''), r.name) ASC
		LIMIT 3
	`)
	if err != nil {
	...
	return StorageUsageSnapshot{TopRuns: runs}
}
